...
class CustomDictionarySource(
    ...
    // Separate key for shortcuts to avoid breaking OptimizedVocabulary's JSON parsing
    private val customShortcutsKey: String = "${customWordsKey}_shortcuts"
    ...
    private fun getCustomShortcuts(): MutableMap<String, String> {
        val jsonString = prefs.getString(customShortcutsKey, null)
        if (jsonString != null) {
            return try {
    ...
    private fun saveCustomShortcuts(shortcuts: Map<String, String>) {
        prefs.edit().putString(customShortcutsKey, gson.toJson(shortcuts)).apply()
    }
    ...
    override suspend fun getAllWords(): List<DictionaryWord> = withContext(Dispatchers.IO) {
        ...
        getCustomWords()
            .map { (word, freq) ->
                DictionaryWord(word, freq, WordSource.CUSTOM, true, shortcuts[word])
            }
            .sorted()
    }
    ...
    override suspend fun addWord(word: String, frequency: Int, shortcut: String?) {
        ...
        if (shortcut.isNullOrBlank()) {
            shortcuts.remove(word)
        } else {
            shortcuts[word] = shortcut
        }
    ...
    override suspend fun updateWord(oldWord: String, newWord: String, frequency: Int, shortcut: String?) {
        ...
        val shortcuts = getCustomShortcuts()
        shortcuts.remove(oldWord)
        if (shortcut.isNullOrBlank()) {
            shortcuts.remove(newWord)
        } else {
            shortcuts[newWord] = shortcut
        }
        saveCustomShortcuts(shortcuts)
    }
